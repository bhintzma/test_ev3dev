...
def run1(state):
    if state:
        ...
        # Run1
        # M13: Platooning Trucks - 1st Truck - 10 Points
        run1A = Thread(target=Trun1A)
        run1A.start()
        move_steering.on_for_degrees(0, 50, 180)    
        turnLineDetect('C', 25, 3, 'Black', True)
        turnLineDetect('B', 15, 3, 'Black', False)
        turnLineDetect('B', 15, 3, 'White', True)
        PLF_LineDetect1(3, 1)
        move_steering.on_for_degrees(0, 50, 240)
        LWheel.on_for_degrees(30, 175)
        PLF_Degrees1(3, 1, 100)
        WheelShutdown()
        run1B = Thread(target=Trun1B)
        run1B.start()
        PLF_Degrees2(3, 1, 340)
        WheelShutdown()
        # M14 - Bridge - West Side - 10 Points
        FrontMotor.on_for_degrees(50, 30)
        PLF_Degrees3(3, 1, 370)
        WheelShutdown()
        ...
        WheelShutdown()
        sleep(0.1)
        driveStraight(70, 1060)
        WheelShutdown()
        sleep(0.1) 
        lineSquare(10, 'Black', 'Left', 0.3)
        lineSquare(-10, 'White', 'Left', 0.3)
        move_tank.on_for_degrees(-35, -35, 125)
        WheelShutdown()
        move_tank.on_for_degrees(10, -10, 169)
        WheelShutdown()
        run1D = Thread(target=Trun1D)
        run1D.start()
        move_tank.on_for_degrees(35, 35, 310)
        WheelShutdown()
        move_tank.on_for_degrees(-10, -10, 20)
        WheelShutdown()
        FrontMotor.on_for_degrees(90, 125)
        FrontMotorShutdown()
        sleep(0.1)
        # M09: Train Tracks - Repair the Track - 20 Points
        move_tank.on_for_degrees(15, -15, 185)
        WheelShutdown()
        move_tank.on_for_degrees(-35, -35, 90)
        WheelShutdown()
        BackMotor.on_for_degrees(-20, 130)
        BackMotorShutdown()
        move_tank.on_for_degrees(40, 40, 180)
        WheelShutdown()
        # M14 - Bridge - East Side - 10 Points
        lineSquare(15, 'Black', 'Right', 1.0)
        sleep(0.5)
        run1E = Thread(target=Trun1E)
        run1E.start()
        move_tank.on_for_degrees(15, -15, 50)
        WheelShutdown()
        FrontMotor.on_for_degrees(-30, 30)
        FrontMotorShutdown()
        move_tank.on_for_degrees(50, 50, 450)
        WheelShutdown()
        # M02 - Unused Capacity - 30 Points
        FrontMotor.on_for_degrees(30, 30)
        FrontMotorShutdown()
        lineDetect(25, 2, 'Black', False)
        lineDetect(25, 2, 'White', True)
        RWheel.on_for_degrees(25, 90)
        RWheelShutdown()
        PLF_Degrees3(2, -1, 800)
        PLF_Degrees1(2, -1, 200)
        driveStraight(90, 1800)
        WheelShutdown()
...
def run2(state):
    if state:
        ...
        # Run 2
        # MO5: Switch Engine - 20 Points + M16: Cargo Connect - Green Cargo/Circle - 60 Points
