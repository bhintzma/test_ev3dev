...
def motorStall(MotorPort, Speed, StallSpeed):
    ...
    if (MotorPort == 'A'):
        FrontMotor.on(Speed)
        sleep(0.15)
        while (abs(FrontMotor.SpeedPercent) >= abs(StallSpeed)):
            sleep(0.001)
        FrontMotorShutdown()
        ...
        BackMotor.on(Speed)
        sleep(0.08)
        while (abs(BackMotor.SpeedPercent) > abs(StallSpeed)):
            print("motorSpeed = %.2f" % BackMotor.SpeedPercent, file=sys.stderr)            
            sleep(0.00001)
        print("Stopping BackMotor, motorSpeed = %.2f" % BackMotor.SpeedPercent, file=sys.stderr)
        BackMotorShutdown()
